...
export class MockProvider implements AIProvider {
  ...
  async explainClause(input: ExplainClauseInput): Promise<ClauseAnalysisResult> {
    return {
      ...
      explanation:
        "This is a mock explanation from the local adapter layer. It proves the selected-clause workflow can call an AIProvider without contacting any real AI service.",
      reviewPoints: [
        "Confirm the clause text was intentionally selected before running any future real provider.",
        "Review commercial and legal context manually; this mock provider does not evaluate legal risk.",
        "Keep deterministic defined-term, cross-reference, and obligation results separate from AI-style commentary.",
      ],
      notes: [
  ...
  async summarizeClause(input: SummarizeClauseInput): Promise<ClauseAnalysisResult> {
    return {
      ...
      explanation:
        "This is a mock summary workflow response. It is included only to satisfy the shared ClauseAnalysisResult shape.",
      reviewPoints: ["No review points were generated by a real provider."],
      notes: [
        "No real AI provider was called.",
